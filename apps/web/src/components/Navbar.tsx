...
export const Navbar = ({ onViewChange, currentView }: NavbarProps) => {
    ...
    const handleNavClick = (e: React.MouseEvent, target: string) => {
        ...
        if (target === "how-it-works") {
            onViewChange("home");
            // Small timeout to allow render if switching views
            setTimeout(() => {
                ...
                <div className="hidden md:flex items-center gap-8">
                    {["Home", "Features", "How it works"].map((item) => (
                        <a
                            ...
                            onClick={(e) => handleNavClick(e, item === "How it works" ? "how-it-works" : item)}
                            className={`text-sm font-medium transition-colors relative group ${currentView === item.toLowerCase() ? "text-[#478EDB]" : "text-slate-500 hover:text-[#478EDB]"
                                }`}
                        >
                            {item}
                            <span className={`absolute -bottom-1 left-0 w-full h-0.5 bg-[#478EDB] transition-transform origin-left duration-300 ${currentView === item.toLowerCase() ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"
                                }`}></span>
                        </a>
                    ))}
                </div>
