...
type Tab = "overview" | "websites" | "pages" | "analytics" | "social" | "visitors" | "settings" | "billing";
    ...
    <div className="min-h-screen bg-[#f8f4ee] pt-14 text-[#1f2522]">
      ...
      <div className="relative z-10 flex min-h-[calc(100vh-56px)]">
        ...
        <main className="min-w-0 flex-1 overflow-auto px-5 pb-24 pt-6 lg:px-8 lg:pb-8 lg:pt-8" style={{ maxHeight: 'calc(100vh - 56px)' }}>
          ...
          {activeTab === "settings"  && <SettingsTab voiceEnabled={voiceEnabled} setVoiceEnabled={setVoiceEnabled} webDataOnly={webDataOnly} setWebDataOnly={setWebDataOnly} activeSite={activeSite} userId={user?.id ?? ""} />}
          {activeTab === "pages"     && <PagesTab activeSite={activeSite} userId={user?.id ?? ""} />}
          {activeTab === "billing" && (
            <BillingTab
...
function DailyLimitPanel({ activeSite, userId }: { activeSite: Website | null; userId: string }) {
...
function PagesTab({ activeSite, userId }: { activeSite: Website | null; userId: string }) {
  ...
  const removeSelected = async () => {
    ...
    setNotice(null);
    try {
      const res = await fetch(`${API_BASE}/api/sites/${activeSite.id}/pages?userId=${encodeURIComponent(userId)}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ urls }),
      });
...
function SettingsTab({ voiceEnabled, setVoiceEnabled, webDataOnly, setWebDataOnly, activeSite, userId }: {
  voiceEnabled: boolean; setVoiceEnabled: (v: boolean) => void;
  webDataOnly: boolean; setWebDataOnly: (v: boolean) => void;
  activeSite: Website | null; userId: string;
}) {
  return (
    <div className="space-y-4">
      <DailyLimitPanel activeSite={activeSite} userId={userId} />
      <div className={`${DASH_PANEL} p-6`}>
