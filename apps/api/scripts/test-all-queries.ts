/**
 * Test all queries from screenshots against the knowledge base.
 * Usage: pnpm --filter api exec tsx scripts/test-all-queries.ts
 */
...
async function main() {
  ...
  console.log(`Testing ${QUERIES.length} queries (starting from ${startFrom})...\n`);

  for (let i = startFrom; i < QUERIES.length; i++) {
    const q = QUERIES[i]!;
    console.log(`[${i + 1}/${QUERIES.length}] ${q.slice(0, 80)}...`);
    ...
    for (let attempt = 0; attempt < 3 && !success; attempt++) {
      try {
        ...
        results.push({
          q,
          method: route.routingMethod,
          slugs,
          answer: result.answer,
        });

        console.log(`  ✓ ${route.routingMethod} → [${slugs.join(", ")}]`);
        success = true;
      } catch (err) {
    ...
    // Rate limit — 15s between queries to stay under 5 RPM
  ...
  for (let i = 0; i < results.length; i++) {
    const r = results[i]!;
    console.log(`\n--- Q${i + 1}: ${r.q}`);
    console.log(`Route: ${r.method} → [${r.slugs.join(", ")}]`);

    if (r.error) {
      console.log(`ERROR: ${r.error.slice(0, 200)}`);
      errors++;
    } else if (
      r.answer.toLowerCase().includes("i don't have that information") ||
      r.answer.toLowerCase().includes("please check plaksha.edu.in")
    ) {
      console.log(`ANSWER: ⚠️  IDK response`);
  ...
  console.log("\n\n" + "=".repeat(100));
  console.log(
    `TOTALS: ${answered} answered | ${idk} IDK | ${errors} errors | ${results.length} total`
  );
  console.log("=".repeat(100));
